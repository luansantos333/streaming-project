...
public interface MovieRepository extends JpaRepository <Movie, Long> {
    ...
    List<Long> searchMoviesById(String name);


    @Query(nativeQuery = true, value = "SELECT * FROM (SELECT movie.id, movie.title, movie.release, movie.director, movie.img_url AS imgUrl, movie.price, genre.id AS genreId, genre.name AS genre_name " +
            "FROM tb_movie AS movie INNER JOIN tb_movie_genre AS movie_genre ON movie.id = movie_genre.movie_id INNER JOIN tb_genre AS genre ON genre.id = movie_genre.genre_id WHERE movie.id IN (:movieIds)) AS selection", countQuery = "SELECT COUNT (*) FROM (SELECT movie.id, movie.title, movie.release, movie.director, movie.img_url AS imgUrl, movie.price, genre.id AS genreId, genre.name AS genre_name " +
            "FROM tb_movie AS movie INNER JOIN tb_movie_genre AS movie_genre ON movie.id = movie_genre.movie_id INNER JOIN tb_genre AS genre ON genre.id = movie_genre.genre_id WHERE movie.id IN (:movieIds)) AS selection")
    Page<MovieGenreProjection> searchAllMoviesAndGenresByMovieIds(List<Long> movieIds, Pageable p);
