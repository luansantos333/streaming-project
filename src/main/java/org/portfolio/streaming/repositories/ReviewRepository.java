...
public interface ReviewRepository extends JpaRepository <Review, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT review.review, review.rating, usuario.email AS username, usuario.id AS userId, movie_tb.id AS movie FROM tb_review AS review INNER JOIN tb_user AS usuario ON review.user_id = usuario.id INNER JOIN tb_movie AS movie_tb ON movie_tb.id = review.movie_id WHERE review.id = :id")
     Optional<UserReviewProjection> searchReviewMovieAndUserByReviewId(Long id);
